...
#define VRAM_D_CR       *(volatile unsigned char*)0x04000243
...
void system_video_init(void) {
    ...
    REG_DISPCNT   = 0x00010003;

    REG_DB_DISPCNT = 0x00010003;
    ...
    while (*text) {
        ...
        if (c < 128) {
            for (int row = 0; row < 8; row++) {
                unsigned char row_data = basic_font[c][row];
...
void update_progress_bar_fill(int progress) {
    int fill_width = (progress * 156) / 100;
    for (int y = 102; y < 111; y++) {
        for (int x = 50; x < 50 + fill_width; x++) {
...
int main(void) {
    ...
    for (int progress = 0; progress <= 100; progress++) {
        ...
        if (progress < 10) {
            ...
            int tens  = progress / 10;
            int units = progress % 10;
        ...
        system_print_text("    ", 112, 130, 0, COLOR_BG);
    ...
    unsigned short previous_keys_state = REG_KEYINPUT;
    ...
    while (!system_authenticated) {
        ...
        if (registered_presses & KEY_RIGHT) {
            int col = active_grid_slot % 3;
            if (col < 2) active_grid_slot++;
            draw_security_pin_grid(active_grid_slot);
        }
        if (registered_presses & KEY_LEFT) {
            int col = active_grid_slot % 3;
            if (col > 0) active_grid_slot--;
            draw_security_pin_grid(active_grid_slot);
        }
        ...
        if (registered_presses & KEY_A) {
            if (active_grid_slot == 11) {
                if (entered_digits_count == 4 &&
                    ...
                    for (int i = 0; i < 4; i++) input_pin_buffer[i] = -1;
                    system_print_text("[ _ ] [ _ ] [ _ ] [ _ ]", 48, 90, 0, COLOR_TEXT);
                }
    ...
    system_print_text("22/05/2026 - 23:49", 104,  8, 0, COLOR_SELECT);
